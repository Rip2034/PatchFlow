...
核心问题：LLM 上下文窗口有限（通常 32K-128K token），
对话轮次多了以后，历史消息会超出窗口限制。

三层压缩策略（不再是粗暴的"只保留最近 N 条"）：
  1. recent_layer — 最近 N 个逻辑回合，完整保留（不丢细节）
  2. medium_layer — 较早回合，压缩工具结果（源代码 → 摘要）
  3. old_layer — 最早回合，整组合并为一段摘要

设计目标：
  - 不再丢信息！老的内容被压缩为一句话但不会完全消失
  - 关键信息（错误消息、文件内容）优先保留
  - 工具调用结果（大段输出）优先压缩
"""
